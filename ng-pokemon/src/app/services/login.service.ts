...
export class LoginService {
  ...
  public login(username: string): Observable<User> {
    // The login function returns an Observable, of type User (as described by the model).
    return this.checkUsername(username).pipe(
      switchMap((user: User | undefined) => {
        // Switches to createUser
        if (user === undefined) {
  ...
  private checkUsername(username: string): Observable<User | undefined> {
    // We need to convert from array to either User or undefined (if user doesn't exist).
    return this.http
      .get<User[]>(`${apiUsers}?username=${username}`) // Always gets an array
      .pipe(
        // RxJS operators
